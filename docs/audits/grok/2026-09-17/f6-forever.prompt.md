...
7. FOREVER COMPARISON — oldExpectation must describe the original first year of World of Warcraft (2004–2005), foreverVersion must describe only what Blizzard officially announced for Forever (no beta observations, no speculation presented as fact), whyItMatters must not add facts, and the schedule entry's dates and times must match the official announcements exactly (check the live pages).
...
# Items to check
Every file in src/content/forever/*.md (11 entries). Ledger: docs/research/evidence/forever.json plus any entry whose entryRefs contains "forever/<id>".
