...
7. DATING — for the timeline: a dateLabel more precise than the sources allow, an "approximate": false on a date the sources give only approximately, a dateNote that misstates why dates differ, a sortKey order that contradicts the sources, a wrong eraId (eras are defined in src/lib/eras.ts), or characterIds/factionIds/locationIds that the event text and quotes do not justify.
...
# Items to check
Every event in src/content/timeline/core.json (32 events). Ledger entries use entryRefs "timeline/<id>"; the chapter ledgers docs/research/evidence/chapter-*.json may also back events.
