...
7. DEFINITIONS — a term definition that is wrong for the original first year of World of Warcraft or for Forever, or that presents a later Retail meaning as the only meaning.
...
# Items to check
Every term in src/content/glossary/core.json (18 terms). Ledger: docs/research/evidence/glossary.json plus any entry whose entryRefs contains "glossary/<id>". Terms about the encyclopedia itself (for example lore statuses, dating conventions) make no lore claim beyond their definition — check only that they do not state lore facts without support.
